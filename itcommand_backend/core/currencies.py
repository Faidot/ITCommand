"""ISO 4217 codes accepted for new subscription currency values.

The list is the unique alphabetic-code set from the ISO 4217 Maintenance
Agency's "List One: Current Currency & Funds", published 2026-01-01.  Keeping
it local makes validation deterministic and avoids a runtime network request or
an additional package dependency.
"""
...
def is_current_iso_4217_code(value):
    return isinstance(value, str) and value.strip().upper() in ISO_4217_CODES
