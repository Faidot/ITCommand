...
Docker deployments run these through the long-lived `automation` service. It
persists successful daily/monthly run markers so restarts do not duplicate work:
...
For a non-Docker deployment, invoke the management commands from a systemd
timer or cron instead (examples):
...
- **`email_finance_report`** — builds a monthly summary (budget, approved spend, this-month income/expense, net, pending approvals, upcoming bills) and emails Admins/Superadmins. Local `DEBUG=True` development defaults to the console email backend; production delivery failures make the command fail and retry rather than reporting a false success.
...
- A dedicated `/subscriptions` workspace tracks any cloud, AI, or SaaS platform, including AWS, Azure, GCP, ChatGPT, Claude, Slack, Notion, Figma, and custom vendors.
- Each record captures platform and plan, monthly/yearly cost and ISO currency, start/expiry dates, category, purpose, team/department, business owner, platform administrator, service URL, auto-renew status, cancellation deadline, and internal notes.
- The dashboard normalizes active spend to monthly and yearly equivalents, separates currencies, charts spend by category, counts active/expired records, and lists renewals due in the next 60 days.
- Renewal, expiry, cancellation, and monthly/yearly budget alerts are generated by `check_subscription_alerts`. Delivery is permission-scoped and idempotent; the frontend refreshes the notification bell periodically and can show optional browser desktop alerts while IT Command is open.
- Role permissions use the `subscriptions` RBAC module. Add/edit/delete controls and APIs fail closed, while subscription-scoped owner/department options avoid requiring broader Users access.
- Accounting exports are available as real PDF and Excel files from `/api/subscriptions/export/?format=pdf|xlsx`; text cells are protected against spreadsheet-formula injection.
- API: `/api/subscriptions/` (CRUD/filter/search), `/dashboard/`, `/options/`, `/settings/`, and `/export/`.
...
- **Notifications** (`Notification`): per-user messages (type, message, link, read flag) surfaced in a polling bell UI, with optional desktop alerts for new subscription reminders — used by finance approvals, auto-post, warranty/contract/license/subscription alerts.
